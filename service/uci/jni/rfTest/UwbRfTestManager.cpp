...
const char* PERIODIC_TX_DATA_CLASS_NAME = "com/android/uwb/test/UwbTestPeriodicTxResult";
const char* PER_RX_DATA_CLASS_NAME = "com/android/uwb/test/UwbTestRxPacketErrorRateResult";
const char* UWB_LOOPBACK_DATA_CLASS_NAME = "com/android/uwb/test/UwbTestLoopBackTestResult";
...
bool IsRfTestOngoing = false; // to track the RF test status whether test is sussesffuly completed or not
...
void UwbRfTestManager::onPerRxDataNotificationReceived(uint16_t len, uint8_t* data) {
    ...
    if(len != 0) {
        ...
        jmethodID perRxCtor = env->GetMethodID(mPerRxDataClass, "<init>", "(IJJJJJJJJJJJJJ)V");
...
void UwbRfTestManager::onLoopBackTestDataNotificationReceived(uint16_t len, uint8_t* data) {
    ...
    if(len != 0) {
        ...
        jmethodID uwbLoopBackCtor = env->GetMethodID(mUwbLoopBackDataClass, "<init>", "(IJIJIIII[B)V");
...
void UwbRfTestManager::onRxTestDataNotificationReceived(uint16_t len, uint8_t* data) {
    ...
    if(len != 0) {
        ...
        jmethodID rxDataCtor = env->GetMethodID(mRxDataClass, "<init>", "(IJIIIII[B)V");
...
void UwbRfTestManager::doLoadSymbols(JNIEnv* env, jobject thiz) {
    ...
    if (clazz != NULL) {
        ...
        mOnPeriodicTxDataNotificationReceived =
            env->GetMethodID(clazz, "onPeriodicTxDataNotificationReceived", "(Lcom/android/uwb/test/UwbTestPeriodicTxResult;)V");
        mOnPerRxDataNotificationReceived =
            env->GetMethodID(clazz, "onPerRxDataNotificationReceived", "(Lcom/android/uwb/test/UwbTestRxPacketErrorRateResult;)V");
        mOnLoopBackTestDataNotificationReceived =
            env->GetMethodID(clazz, "onLoopBackTestDataNotificationReceived", "(Lcom/android/uwb/test/UwbTestLoopBackTestResult;)V");
        mOnRxTestDataNotificationReceived =
            env->GetMethodID(clazz, "onRxTestDataNotificationReceived", "(Lcom/android/uwb/test/UwbTestRxResult;)V");
